...
// 
/**
 * Fixing typeof takes value and returns type of value
 * 
 * @param value
 *            return typeof value
 */
...
/**
 * Iterate over array of objects and call given callback for each item in the
 * array Optionally may take this as scope
 * 
 * @param array
 * @param callback
 * @param optional
...
/**
 * The jdd object handles all of the functions for the main page. It finds the
 * diffs and manages the interactions of displaying them.
 */
/* global jdd:true */
var jdd = {
	...
	btnClickCount:0,
	/**
	 * Find the differences between the two objects and recurse into their sub
	 * objects.
	 */
	findDiffs : function(/* Object */config1, /* Object */data1, /* Object */
			config2, /* Object */data2) {
		...
		if (data1.length < data2.length) {
			/*
			 * This means the second data has more properties than the first. We
			 * need to find the extra ones and create diffs for them.
			 */
			for (key in data2) {
		...
		for (key in data1) {
			if (data1.hasOwnProperty(key)) {
				...
				if (!data2.hasOwnProperty(key)) {
					/*
					 * This means that the first data has a property which isn't
					 * present in the second data
					 */
					jdd.diffs
							.push(jdd
									.generateDiff(
		...
		/*
		 * Now we want to look at all the properties in object two that weren't
		 * in object one and generate diffs for them.
		 */
		for (key in data2) {
			if (data2.hasOwnProperty(key)) {
				// no un-used vars
				...
				if (!data1.hasOwnProperty(key)) {
					jdd.diffs.push(jdd.generateDiff(config1, jdd
							.generatePath(config1), config2, jdd.generatePath(
							config2, key), 'Missing property <code>' + key
	...
	/**
	 * Generate the differences between two values. This handles differences of
	 * object types and actual values.
	 */
	diffVal : function(val1, config1, val2, config2) {
		...
		if (getType(val1) === 'array') {
			...
			if ([ 'array', 'string', 'number', 'boolean', 'null' ]
					.indexOf(getType(val2)) > -1) {
				jdd.diffs.push(jdd.generateDiff(config1, jdd
						.generatePath(config1), config2, jdd
						.generatePath(config2), 'Both types should be objects',
						jdd.TYPE));
			} else {
			...
			if (getType(val2) !== 'string') {
				jdd.diffs.push(jdd.generateDiff(config1, jdd
						.generatePath(config1), config2, jdd
						.generatePath(config2), 'Both types should be strings',
						jdd.TYPE));
			} else if (val1 !== val2) {
				jdd.diffs.push(jdd.generateDiff(config1, jdd
						.generatePath(config1), config2, jdd
			...
			if (getType(val2) !== 'number') {
				jdd.diffs.push(jdd.generateDiff(config1, jdd
						.generatePath(config1), config2, jdd
						.generatePath(config2), 'Both types should be numbers',
						jdd.TYPE));
			} else if (val1 !== val2) {
				jdd.diffs.push(jdd.generateDiff(config1, jdd
						.generatePath(config1), config2, jdd
	...
	/**
	 * Arrays are more complex because we need to recurse into them and handle
	 * different length issues so we handle them specially in this function.
	 */
	diffArray : function(val1, config1, val2, config2) {
		...
		if (val1.length < val2.length) {
			...
			for (var i = val1.length; i < val2.length; i++) {
				jdd.diffs.push(jdd.generateDiff(config1, jdd
						.generatePath(config1), config2, jdd.generatePath(
						config2, '[' + i + ']'), 'Missing element <code>' + i
		...
		val1.forEach(function(arrayVal, index) {
			if (val2.length <= index) {
				jdd.diffs.push(jdd.generateDiff(config1, jdd.generatePath(
						config1, '[' + index + ']'), config2, jdd
						.generatePath(config2), 'Missing element <code>'
						+ index + '</code> from the array on the right side',
						jdd.MISSING));
			} else {
	...
	/**
	 * We handle boolean values specially because we can show a nicer message
	 * for them.
	 */
	...
	formatAndDecorate : function(/* Object */config, /* Object */data) {
		...
		/*
		 * If the first set has more than the second then we will catch it when
		 * we compare values. However, if the second has more then we need to
		 * catch that here.
		 */
	...
	/**
	 * Generate the start of the an array in the output stream and push in the
	 * new path
	 */
	startArray : function(config) {
		...
		if (config.paths.length === 0) {
			/*
			 * Then we are at the top of the array and we want to add a path for
			 * it.
			 */
			config.paths.push({
				path : jdd.generatePath(config),
	...
	/**
	 * Generate the start of the an object in the output stream and push in the
	 * new path
	 */
	startObject : function(config) {
		...
		if (config.paths.length === 0) {
			/*
			 * Then we are at the top of the object and we want to add a path
			 * for it.
			 */
			config.paths.push({
				path : jdd.generatePath(config),
	...
	/**
	 * When we parse the JSON string we end up removing the escape strings when
	 * we parse it into objects. This results in invalid JSON if we insert those
	 * strings back into the generated JSON. We also need to look out for
	 * characters that change the line count like new lines and carriage
	 * returns.
	 * 
	 * This function puts those escaped values back when we generate the JSON
	 * output for the well known escape strings in JSON. It handles properties
	 * and values.
	 * 
	 * This function does not handle unicode escapes. Unicode escapes are
	 * optional in JSON and the JSON output is still valid with a unicode
	 * character in it.
	 */
	unescapeString : function(val) {
		if (val) {
			return val.replace('\\', '\\\\') // Single slashes need to be
												// replaced first
	...
	/**
	 * Generate a JSON path based on the specific configuration and an optional
	 * property.
	 */
	...
	/**
	 * Sort all the relevant properties and return them in an alphabetical sort
	 * by property key
	 */
		...
		forEach($('pre'), function(pre) {
			...
			var addLine = function(line, index) {
				var div = $('<div class="codeLine line' + (index + 1)
						+ '"></div>');
				lineNumbers.append($('<span class="line-number">' + (index + 1)
						+ '.</span>'));
	...
	handleDiffClick : function(line, side) {
		...
		diffs.forEach(function(diff) {
			$('pre.left div.line' + diff.path1.line + ' span.code').addClass(
					'selected');
			$('pre.right div.line' + diff.path2.line + ' span.code').addClass(
					'selected');
		});
	...
	scrollToDiff : function(diff) {
		$('html, body').animate(
				{
					scrollTop : $(
							'pre.left div.line' + diff.path1.line
									+ ' span.code').offset().top
				}, 0);
	},
	...
	generateReport : function() {
		var report = $('#report');
		report.text('');
		var hide = $('<button>Show Only Diff</button>');
		report.append(hide);
		hide.click(function(e) {
			if (jdd.btnClickCount==0) {
			jdd.hideLine();
			jdd.btnClickCount=1;
			hide.html("Show All Rows")
			}
			else {
				jdd.showLine();
				jdd.btnClickCount=0;
				hide.html("Show Only Diff")
			}
		});

		if (jdd.diffs.length === 0) {
			report
					.append('<span>The two files were semantically  identical.</span>');
			return;
		}
		...
		if (missingCount > 0) {
			...
			missing.children('input').click(
					function() {
						if (!$(this).prop('checked')) {
							$('span.code.diff.missing').addClass('missing_off')
									.removeClass('missing');
						} else {
							$('span.code.diff.missing_off').addClass('missing')
		...
		if (typeCount > 0) {
			...
			types.children('input').click(
					function() {
						if (!$(this).prop('checked')) {
							$('span.code.diff.type').addClass('type_off')
									.removeClass('type');
						} else {
							$('span.code.diff.type_off').addClass('type')
									.removeClass('type_off');
						}
					});
		...
		if (eqCount > 0) {
			...
			eq.children('input').click(
					function() {
						if (!$(this).prop('checked')) {
							$('span.code.diff.eq').addClass('eq_off')
									.removeClass('eq');
						} else {
							$('span.code.diff.eq_off').addClass('eq')
									.removeClass('eq_off');
						}
					});
	...
	compare : function() {
		...
		if (jdd.requestCount !== 0) {
			/*
			 * This means we have a pending request and we just need to wait for
			 * that to finish.
			 */
			return;
		}
		...
		/*
		 * We'll start by running the text through JSONlint since it gives much
		 * better error messages.
		 */
		var leftValid = jdd.validateInput($('#textarealeft').val(), jdd.LEFT);
		var rightValid = jdd
				.validateInput($('#textarearight').val(), jdd.RIGHT);
	...
	loadSampleData : function() {
		...
		/*
		 * $('#textarealeft').val('[{ "OBJ_ID": "CN=Kate
		 * Smith,OU=Users,OU=Willow,DC=cloudaddc,DC=qalab,DC=cam,DC=novell,DC=com",
		 * "userAccountControl": "512", "objectGUID":
		 * "b3067a77-875b-4208-9ee3-39128adeb654", "lastLogon": "0",
		 * "sAMAccountName": "ksmith", "userPrincipalName":
		 * "ksmith@cloudaddc.qalab.cam.novell.com", "distinguishedName":
		 * "CN=Kate
		 * Smith,OU=Users,OU=Willow,DC=cloudaddc,DC=qalab,DC=cam,DC=novell,DC=com"},{
		 * "OBJ_ID": "CN=Timothy
		 * Swan,OU=Users,OU=Willow,DC=cloudaddc,DC=qalab,DC=cam,DC=novell,DC=com",
		 * "userAccountControl": "512", "objectGUID":
		 * "c3f7dae9-9b4f-4d55-a1ec-bf9ef45061c3", "lastLogon":
		 * "130766915788304915", "sAMAccountName": "tswan", "userPrincipalName":
		 * "tswan@cloudaddc.qalab.cam.novell.com", "distinguishedName":
		 * "CN=Timothy
		 * Swan,OU=Users,OU=Willow,DC=cloudaddc,DC=qalab,DC=cam,DC=novell,DC=com"}]');
		 * $('#textarearight').val('{"foo":[{ "OBJ_ID": "CN=Timothy
		 * Swan,OU=Users,OU=Willow,DC=cloudaddc,DC=qalab,DC=cam,DC=novell,DC=com",
		 * "userAccountControl": "512", "objectGUID":
		 * "c3f7dae9-9b4f-4d55-a1ec-bf9ef45061c3", "lastLogon":
		 * "130766915788304915", "sAMAccountName": "tswan", "userPrincipalName":
